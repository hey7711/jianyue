...
- 设计应用 (How):
  ...
  - UI/UX一致性: 遵循微信小程序的设计规范和交互习惯，使用用户熟悉的组件和模式，降低用户的认知负荷。
...
- 品牌主色 (Primary Color)
  - brand-500: #3B82F6
    - 设计理念: 传递专业、可靠和高效的品牌感受。作为核心行动引导色，鼓励用户完成关键操作，建立对产品的信任感。
    - 使用场景:
      ...
      - 输入框获得焦点时的外发光或边框
  - brand-100: #EFF6FF
    - 设计理念: 作为主色的辅助，提供一种轻量、柔和的视觉反馈，用于表示元素的“被选中”或“悬停”状态，增加界面的亲和力。
    - 使用场景:
      - 列表或表格中被选中的行背景
      - 主要按钮的悬停(Hover)状态背景
      - “已确认”等积极但非关键状态的标签背景
      - 需要轻微突出的信息卡片背景
  - brand-700: #1D4ED8
    - 设计理念: 提供更强的视觉反馈，用于表示元素的“被点击”状态，确保交互行为的即时确认。同时作为高对比度的背景色使用。
    - 使用场景:
      - 主要按钮被点击(Active)时的颜色
      - 在浅色背景上承载白色文字时的背景
...
- 颜色与使用
  - 设计理念: 图标本身不应包含颜色信息，应设计为单色。它的颜色由其应用的上下文决定，通过代码（CSS的 `currentColor` 属性）继承文本颜色，确保图标与旁边的文字颜色始终保持一致和协调。
  - 规范:
    - 默认状态: `text-primary` (#1E293B) 或 `text-secondary` (#64748B)，取决于其重要性。
    - 交互状态: `brand-500` (#3B82F6)，用于可交互的、激活的或选中的图标。
...
- 插图风格 (Illustration Style)
  ...
  - - 色彩运用
    ...
    - 规范:
      - 主色调: 以 `brand-100` (#EFF6FF) 和中性色 `bg-secondary` (#F8FAFC) 作为插图的大面积背景色。
      - 点缀色: 使用 `brand-500` (#3B82F6) 作为视觉焦点和关键元素的颜色。
      - 辅助色: 可引入一个非冲突的辅助色（如柔和的黄色 `#FBBF24`）用于点缀，但使用面积不应超过10%。
  ...
  - - 应用场景
    - Onboarding引导页: 以系列插图生动地介绍产品的核心价值。
...
- 交互原则 (Interaction Principles)
  - 即时反馈 (Instant Feedback)
    - 设计理念: 用户的每一个点击、触摸或输入，都必须在100毫秒内得到视觉响应。这是让用户感觉系统“反应快”而非“卡顿”的心理学基础。
    - 应用场景:
      - 按钮被点击时，立即改变颜色或尺寸。
      - 输入框获得焦点时，立即高亮边框。
...
- 应用场景示例
  ...
  - 列表项添加/删除
    - 添加: 新项目以 300ms 从高度为0展开到正常高度，并淡入。
    - 删除: 被删除项目以 300ms 收缩高度至0并淡出，下方列表项平滑地向上移动补位。
  - 数据加载 (Loading)
...
- 栅格系统 (Grid System)
  - 设计理念: 内容区将采用一个灵活的12列栅格系统。12是一个非常理想的数字，可以被2、3、4、6整除，方便我们将内容区域灵活地划分为等宽的几栏，实现多样化且对齐的布局。
  - 规范:
    - 列数 (Columns): 12
...
- 间距规范 (Spacing System)
  ...
  - 间距量尺 (Spacing Scale):
    ...
    - `space-4`: 16px (m) - 基础间距
      - 用途: 最常用的间距单位。用于区隔独立的、小型的UI组件，如两个相邻的按钮，或卡片内部的水平内边距。
    - `space-6`: 24px (l)
...
- 核心理念
  - 清晰是底线: 确保所有用户都能轻松、无障碍地阅读界面上的文字、识别图标和控件，是所有交互能够发生的基础。一个看不清的按钮等于一个不存在的按钮。
...
- 基于我们色板的应用指南
  - 绝对安全的组合 (✓ 满足 AA / AAA)
    ...
    - 示例:
      ...
      - 白色 (#FFFFFF) on `brand-700` (#1D4ED8) (对比度: 8.59:1)
      - 白色 (#FFFFFF) on `success-500` (#10B981) (对比- 度: 4.26:1, 仅限大型文本或图形)
      - 白色 (#FFFFFF) on `error-500` (#EF4444) (对比度: 3.99:1, 仅限大型文本或图形)
  - 有条件使用的组合 (⚠︎ 需注意)
    - 设计理念: 以下组合仅在满足特定条件（如用于大型文本）时才符合标准，使用时需谨慎检查。
    - 示例:
      - `brand-500` (#3B82F6) on `bg-primary` (#FFFFFF) (对比度: 3.05:1)
        - **规范**: 此组合**不能**用于常规正文。**仅可**用于 H1/H2/H3 等大型文本，或图标、按钮边框等图形元素。
  ...
  - 严禁用于文本的组合 (✗ 不合规)
    ...
    - 示例:
      ...
      - `brand-500` (#3B82F6) 上的白色文字 (#FFFFFF)
        - **规范**: **严禁**将白色文字直接置于 `brand-500` (#3B82F6) 背景上，除非文字是大型的粗体。对于我们的主要按钮，应使用 `brand-700` 作为背景，或为 `brand-500` 添加深色描边。 *（此处更正：白色在 #3B82F6 上的对比度为 3.05:1，因此仅可用于大型文本）*
